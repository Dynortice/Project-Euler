HAND_RANKS <- list("High Card" = 0, "One Pair" = 1, "Two Pairs" = 2, "Three of a Kind" = 3, "Straight" = 4, "Flush" = 5,
                   "Full House" = 6, "Four of a Kind" = 7, "Straight Flush" = 8, "Royal Flush" = 9)

CARD_RANKS <- list("2" = 0, "3" = 1, "4" = 2, "5" = 3, "6" = 4, "7" = 5, "8" = 6, "9" = 7, "T" = 8, "J" = 9, "Q" = 10,
                   "K" = 11, "A" = 12)
...
compute <- function(path) {
    ...
    is.royal <- function(values) {
        return(sum(values) == 50)
    }

    is.consecutive <- function(values, hand_value) {
        return(sum(values) == 5 * (hand_value - 2))
    }

    get_rank <- function(cards) {
        values <- unlist(CARD_RANKS[substr(cards, 1, 1)])
        suits <- substr(cards, 2, 2)
        counted_values <- table(values)
        ...
        if (unique_values == 5) {
            ...
            if (unique_values == 4) {
                ...
                if (max(counted_values) == 3) {
                    rank <- "Three of a Kind"
                } else {
                    rank <- "Two Pairs"
                }
            } else {
                if (max(counted_values) == 4) {
                    rank <- "Four of a Kind"
                } else {
                    rank <- "Full House"
                }
    ...
    is.winner <- function(hands) {
        player_1 <- get_rank(hands[1:5])
        player_2 <- get_rank(hands[6:10])
